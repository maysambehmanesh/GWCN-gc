...
def data_loader2(dataset,batch,node_level=False):
    
    packed = pack(dataset,batch, return_dict=True)
    y = None
    if "y" in dataset.signature:
        y = packed.pop("y_list")
        if node_level:
...
def pack(dataset,batch, return_dict=False):
        output = [list(elem) for elem in zip(*[g.numpy() for g in batch])]
        if return_dict:
            keys = [k + "_list" for k in dataset.signature.keys()]
...
def computeLoaderCombine(data_tr,appriximate_phsi,N_scales,scales,m,epochs,thr):
    ...
    # itrData_tr=data_loader(dataset)
    itrData_tr=data_loader2(data_tr,data_tr)
    ...
    if appriximate_phsi == True:
        psi,psi_inv=approximate_Psi2(L,N_scales,m)
    else:
        # psi,psi_inv=compute_Psi(L,scales)
        psi,psi_inv=compute_Psi2(L,scales,weight_sparse,weight_normalize,thr)
    ...
    data_adj2=[]
    data_adj2.append(data_adj[0])
    data_adj2.append(psi)
    data_adj2.append(psi_inv)
    data_adj2.append(data_adj[1])
    data_adj2.append(data_adj[2])
    data_adj=tuple(data_adj2)
    ...
    return newLoader_tr,psi,psi_inv
...
def laplacian3(W, normalized=False):
...
def approximate_Psi2(L,N_scales,m):
    """
    approximate wavelet with Chebychev polynomial
    Input:
        L: sparse Laplacian matrix
...
def compute_Psi3(L,scales):
    ...
    psi=weight_wavelet(scales[0],lamb,U)
    psi_inv=weight_wavelet_inverse(scales[0],lamb,U)
...
def compute_Psi2(L,scales,weight_sparse,weight_normalize,threshold):
    """
    compute wavelet 
    Input:
        L: sparse Laplacian matrix
    ...
    for s in range(N_scales):
        weight=weight_wavelet(scales[s],lamb,U)
        weight_inverse=weight_wavelet_inverse(scales[s],lamb,U)
        
        #  make it sparse
        if (weight_sparse == True):
            weight[weight < threshold] = 0.0
            weight_inverse[weight_inverse < threshold] = 0.0
        #  make it normalize
        if (weight_normalize == True):
            weight = normalize(weight, norm='l1', axis=1)
            weight_inverse = normalize(weight_inverse, norm='l1', axis=1)
        
        psi.append(weight)
        psi_inv.append(weight_inverse)
    del U,lamb
